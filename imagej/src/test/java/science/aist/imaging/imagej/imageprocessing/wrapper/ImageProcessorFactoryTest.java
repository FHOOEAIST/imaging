...
public class ImageProcessorFactoryTest {
    ...
    public void testGetImage2() {
        ...
        int width = 10;
        int height = 15;
        ImageProcessor img = new ShortProcessor(width, height);
        Random rand = new Random(768457);

        int[] channelArr = new int[1];
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                channelArr[0] = rand.nextInt(255);
                img.putPixel(x, y, channelArr);
        ...
        // when
        ImageWrapper<ImageProcessor> image = imageProcessorFactory.getImage(height, width, ChannelType.GREYSCALE, img);
