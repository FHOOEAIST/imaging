...
import science.aist.imaging.opencv.imageprocessing.domain.ThinningType;
...
public class ThinningFunction implements ImageFunction<Mat, Mat> {
    ...
    public ImageWrapper<Mat> apply(ImageWrapper<Mat> matImageWrapper) {
        ImageWrapper<Mat> result = ImageFactoryFactory.getImageFactory(Mat.class).getImage(matImageWrapper.getWidth(), matImageWrapper.getHeight(), matImageWrapper.getChannelType());
